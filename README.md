...
``` sh
java -cp  target/inaccurate-article-crawler-1.0-SNAPSHOT.jar crawlercommons.urlfrontier.client.Client PutURLs -f seeds.txt
```
...
``` sh
storm local target/inaccurate-article-crawler-1.0-SNAPSHOT.jar  org.apache.storm.flux.Flux crawler.flux --local-ttl 3600
```
